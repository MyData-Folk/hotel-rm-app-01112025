...
def create_hotel(hotel_id: str = Query(..., min_length=3)):
    ...
    with Session(engine) as session:
        ...
        # Create dedicated directory for the hotel
        hotel_dir = os.path.join(DATA_DIR, hotel_id)
        os.makedirs(hotel_dir, exist_ok=True)

        logger.info(f"Hôtel créé: {hotel_id} et dossier initialisé.")
...
async def upload_excel(hotel_id: str = Query(...), file: UploadFile = File(...)):
    ...
    try:
        ...
        out_path = os.path.join(hotel_dir, 'data.json')
...
def get_data(hotel_id: str = Query(...)):
    hotel_id = decode_hotel_id(hotel_id)
    path = os.path.join(DATA_DIR, hotel_id, 'data.json')
